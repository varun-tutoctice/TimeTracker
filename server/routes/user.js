   ...
   // Check for registration errors
    ...
    User.findOne({ email: email }, function(err, existingUser) {
        ...
        oUser.save(function(err, oUser) {
            ...
            res.status(201).json({
                success: true,
		message: 'User created successfully, login to access your account.'
            });
        });
    });
}
		...
		if (!user) {
			...
			expiresIn: config.tokenexp
                    ...
                    user.save(function(err) {
                        ...
                        res.status(201).json({
                            success: true,
                            message: {'userid': user._id, 'email': user.email, 'firstname': user.firstname, 'lastlogin': last_login},
                            token: token
                        });
                    });
                } else {
...
exports.getusercount = function(req, res, next){
    User.count().exec(function(err, user){
        if(err){ res.status(400).json({ success: false, message: 'Error processing request '+ err}); }
        res.status(201).json({
		success: true, 
		data: user
